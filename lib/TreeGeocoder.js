...
class TreeGeocoder {
    ...
    async geocode(opts) {
        ...
        const suffixClient = new TreeBrowser.FuzzyAutocompleteClient(100, 2, opts.minScore);
        ...
        if (opts.stream) {
            ...
            try {
                ...
                prefixClient.on('data', res => {
                    results = results.concat(this.extractAndFilterResults([res], pathSet, opts.filter, opts.minScore, unique));
                });
                ...
                console.error(`took ${new Date() - t0} ms`);
